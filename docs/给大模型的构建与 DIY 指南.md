...
| 深改菜单/布局/欢迎页 | `desktop/vscode/src/vs/workbench/...`（克隆后） | 全量 `build-win32.ps1`；壳层改造见 `docs/code-oss-build.md` |
...
说明见 `grok/README.md`。自建引擎可参考上游 [xai-org/grok-build](https://github.com/xai-org/grok-build)。
...
2. 阅读 `docs/code-oss-build.md`，并对照 `desktop/scripts/` 里已有补丁先例。
3. 源码改动加注释标记便于日后合并上游。
...
- `docs/功能能力清单.md` — 功能列表  
- `grok/README.md` — 二进制获取（自建见上游 grok-build）  
